    ...
    func setupUI() {
        let size = frame.height
            ...
            if semanticContentAttribute == .forceRightToLeft {
                leftView = arrowContainerView
                leftViewMode = .always
            } else {
                rightView = arrowContainerView
                rightViewMode = .always
            }
            
            let arrowX: CGFloat = frame.width - size
            let arrowY: CGFloat = 0
            let arrowSize: CGFloat = size
            arrow = Arrow(origin: CGPoint(x: arrowX, y: arrowY), size: arrowSize)
